...
/// invalid unroll factors. In case of unroll factor of 1, the function bails out
/// without doing anything (returns success).
/// Currently, only constant trip count that are divided by
/// the unroll factor is supported. Currently, for operations with results are
/// not supported.
